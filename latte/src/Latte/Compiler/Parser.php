...
class Parser
{
	...
	private function contextMacro(): void
	{
		...
		if (!empty($matches['macro'])) {
			...
			$macroTag = $this->parseMacroTag($matches['macro']);
			$token->name = $macroTag[0];
			$token->value = $macroTag[1];
			$token->modifiers = $macroTag[2];
			$token->empty = $macroTag[3];
			$token->closing = $macroTag[4];
